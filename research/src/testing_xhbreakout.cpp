...
)
{
    ...
    definitions.push_back(StrategyDefinition{
        ...
        [=](const ParameterValues& parameters) {
            const unsigned int xH = unsignedParameterOr(
                parameters,
                "xH",
                30U
            );
    ...
    definitions.push_back(StrategyDefinition{
        ...
        makeSensitivityParameters({
            {"xH", "XH lookback", 10.0, 100.0, 5.0, true},
            {"atrLength", "ATR length", 10.0, 30.0, 2.0, true},
            {"atrMultiplier", "ATR stop multiplier", 1.0, 10.0, 1.0, false},
            {"momentumLength", "Momentum length", 20.0, 60.0, 4.0, true},
            {"quantityPercent", "Quantity (%)", 2.0, 20.0, 2.0, false},
            {"maxPositionsOpen", "Maximum open positions", 1.0, 20.0, 1.0, false}
        }),
        ...
        [](const ParameterValues& parameters) {
            ...
            return xH > 0U &&
                   atrLength > 0U &&
                   std::isfinite(atrMultiplier) &&
                   atrMultiplier > 0.0 &&
                   momentumLength > 0U &&
                   std::isfinite(quantityPercent) &&
                   quantityPercent > 0.0 &&
                   maxPositionsOpen > 0U;
        }
    });
...
)
{
    ...
    writeCsvRow(
        output,
        {
            ...
            "periods_per_year",
            "database_timeframe",
            "annualization_benchmark_symbol",
    ...
    writeCsvRow(
        output,
        {
            ...
            metricsSettings.databaseTimeframe,
            metricsSettings.annualizationBenchmarkSymbol,
...
double periodsPerYearFromTimeframe(const std::string& timeframe)
{
    if (timeframe == "1d") {
        return 365.0;
    }

    if (timeframe.size() >= 2U && timeframe.back() == 'h') {
        const int hours = std::stoi(timeframe.substr(0, timeframe.size() - 1U));
        ...
        if (
            ...
            return 365.0 * 24.0 / static_cast<double>(hours);
        }
    }

    throw std::runtime_error(
        "Unsupported database timeframe: " + timeframe +
        ". Use 1d, 1h, 2h, 3h, 4h, 6h, 8h or 12h."
    );
}
